...
type (
	...
	apiFees struct {
		...
		Fee     scprimetypes.Currency `json:"fee"`
	}

	getFeesResp struct {
		APIResponse
		Minimum scprimetypes.Currency `json:"minimum"`
		Maximum scprimetypes.Currency `json:"maximum"`
	...
	GetTransactionsResp struct {
		APIResponse
		UnspentSiacoins         scprimetypes.Currency `json:"unspent_siacoins"`
		UnspentSiafunds         scprimetypes.Currency `json:"unspent_siafunds"`
...
func (a *APIClient) GetTransactionFees() (min, max scprimetypes.Currency, err error) {
...
func (a *APIClient) GetAPIFees() (fee scprimetypes.Currency, address string, err error) {
	var resp getFeesResp
...
func (a *APIClient) BroadcastTransactionSet(transactions []scprimetypes.Transaction) (err error) {
